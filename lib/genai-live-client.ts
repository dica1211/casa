...
export interface LiveClientEventTypes {
  ...
  turncomplete: [];
  // Emitted when text data is received
  text: [text: string];
  // Emitted when grounding metadata is received
  groundingMetadata: [chunks: GroundingChunk[]];
}
...
export class GenAILiveClient extends EventEmitter<LiveClientEventTypes> {
  ...
  protected onMessage(message: LiveServerMessage & { groundingMetadata?: { groundingChunks: GroundingChunk[] } }) {
    ...
    if (message.groundingMetadata && message.groundingMetadata.groundingChunks) {
      this.log('server.groundingMetadata', message.groundingMetadata);
      this.emit('groundingMetadata', message.groundingMetadata.groundingChunks);
    }
    ...
    if (message.serverContent) {
      ...
      if (serverContent.modelTurn) {
        ...
        const otherParts = difference(parts, audioParts, textParts);

        // Fix: `base64s` was not defined. Initialize it from `audioParts`.
